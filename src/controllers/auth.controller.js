...
const login = async (req, res) => {
    ...
    try {
        ...
        // If the password is correct, generate a token and send it to the user
            ...
            return res.status(200).json({ success: true, message: "Logged-in successfully!", token: token, role: existingUser.role, level: existingUser.level, streak: existingUser.streak });
        }
    } catch (error) {
        return res.status(500).json({ success: false, message: error.message });
    }
